...
def train(data_dir, model_dir, args):
    ...
    for epoch in range(args['epochs']):
        ...
        for idx, train_batch in enumerate(train_loader):
            ...
            if args['model'] == 'EffiNet01MultiHead':
                mask_preds = torch.argmax(outs[0], dim=-1)
                gender_preds = torch.argmax(outs[1], dim=-1)
                age_preds = torch.argmax(outs[2], dim=-1)
                preds = torch.stack([mask_preds, gender_preds, age_preds],dim=1)
                loss[0].backward()
                optimizer.step()
                loss_value += loss[0].item()
                matches += (preds == labels).sum().item() // 3
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                if args['model'] == 'EffiNet01MultiHead':
                    ...
                    preds = torch.stack([mask_preds, gender_preds, age_preds], dim=1)
                    loss_value += loss[0].item()
                    acc_item = (labels == preds).sum().item() / 3
...
def get_params(parser):
    ...
    parser.add_argument('--model', type=str, default="EffiNet01", help='model type (default: BaseModel)')
...
if __name__ == '__main__':
    try:
        ...
        if nni_applied:
            ...
            tuner_params = get_params(parser)
            tuner_params2 = dict({"epochs":5, "lr":1e-4, "criterion": "label_smoothing", "batch_size": 32, "val_ratio": 0.01, 'optimizer': 'Adam', 'model': 'EffiNet01'})
            value = merge_parameter(tuner_params, tuner_params2)
            args = vars(value)
            print(args)
            data_dir = args['data_dir']
            model_dir = args['model_dir']
